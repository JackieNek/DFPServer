module.exports = db => {
  ...
  function addUserOn(data, callback) {
    ...
    Record.findOneAndUpdate({ _id: new ObjectID(data.recordId) }, { $push: { userOn: user }}, (err, docs) => {
      callback(err, docs);
    });
  }
  ...
  function removeUserOn(data, callback) {
    ...
    Record.findOneAndUpdate({ _id: new ObjectID(data.recordId) }, { $pull: { userOn: user }}, (err, docs) => {
      callback(err, docs);
    });
  }
}